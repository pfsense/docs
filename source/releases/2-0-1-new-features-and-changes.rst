...
-  Fixes for RRD upgrade/migration and backup
   (`#1758 <http://redmine.pfsense.org/issues/1758>`__)
...
-  Fixed uname display in System Info widget
   (`#1960 <http://redmine.pfsense.org/issues/1960>`__)
...
-  Clarified text of serial field when importing a CA
   (`#2031 <http://redmine.pfsense.org/issues/2031>`__)
-  Fixed MTU setting on upgrade from 1.2.3, now upgrades properly as MSS
   adjustment (`#1886 <http://redmine.pfsense.org/issues/1886>`__)
...
-  Fixed situation where certain NICs would constantly cycle link with
   ...
   (`#1572 <http://redmine.pfsense.org/issues/1572>`__)
-  Fixed OpenVPN ordering of client/server IPs in Client-Specific
   Override entries (`#2004 <http://redmine.pfsense.org/issues/2004>`__)
-  Fixed handling of OpenVPN client bandwidth limit option
-  Fixed handling of LDAP certificates
   (`#2018 <http://redmine.pfsense.org/issues/2018>`__,
   `#1052 <http://redmine.pfsense.org/issues/1052>`__,
   `#1927 <http://redmine.pfsense.org/issues/1927>`__)
...
-  Fixed handling of hostnames in DHCP that start with a number
   (`#2020 <http://redmine.pfsense.org/issues/2020>`__)
-  Fixed saving of multiple dynamic gateways
   (`#1993 <http://redmine.pfsense.org/issues/1993>`__)
...
-  Fixed use of quick on internal DHCP rules so DHCP traffic is allowed
   properly (`#2041 <http://redmine.pfsense.org/issues/2041>`__)
-  Updated ISC DHCP server to 4.2.3
   (`#1888 <http://redmine.pfsense.org/issues/1888>`__) – this fixes a
...
-  Clarified text for media selection
   (`#1910 <http://redmine.pfsense.org/issues/1910>`__)
